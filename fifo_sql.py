...
SELECT 
    ...
    (price - ROUND((cumul_sold_cost - COALESCE(LAG(cumul_sold_cost) OVER w, 0)) / amount_sold, 2)) * amount_sold AS total_pnl,
    --
    ROUND((price - ROUND((cumul_sold_cost - COALESCE(LAG(cumul_sold_cost) OVER w, 0)) / amount_sold, 2)) / ROUND((cumul_sold_cost - COALESCE(LAG(cumul_sold_cost) OVER w, 0)) / amount_sold, 2) * 100) AS pnl_percentage
...
FROM (
  SELECT 
    ...
    amount_sold,  
    price,
    ROUND(prev_cumul_cost + ((sold_amounts.cumul_sold - bought_amounts.prev_cumul_bought)
        / (bought_amounts.cumul_bought - bought_amounts.prev_cumul_bought)) 
        * (cumul_cost - prev_cumul_cost), 2) 
        AS cumul_sold_cost
...
-- This selects the bought amounts
(
    SELECT 
        ticker,
...
AND 
    sold_amounts.cumul_sold <= bought_amounts.cumul_bought
AND 
    sold_amounts.ticker = bought_amounts.ticker
) t

WINDOW w AS (PARTITION BY ticker ORDER BY datetime)
;
"""